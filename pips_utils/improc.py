...
def preprocess_color_tf(x):
    ...
    return tf.cast(x, tf.float32) * 1.0 / 255 - 0.5
...
def xy2heatmap(xy, sigma, grid_xs, grid_ys, norm=False):
    ...
    term1 = 1.0 / 2.0 * np.pi * sigma_sq
    term2 = torch.exp(-(sq_diff_x + sq_diff_y) / (2.0 * sigma_sq))
    gauss = term1 * term2
...
COLORMAP_FILE = os.path.join(dirname, "bremm.png")
...
class Summ_writer(object):
        ...
        if self.save_this:
            ...
            if feats.ndim == 6:
                ...
                if valids is None:
                    ...
                    feats = pips_utils.basic.reduce_masked_mean(
        ...
        if self.save_this:
            if feat.ndim == 5:  # B x C x D x H x W
                ...
                if valid is None:
                    ...
                    feat = pips_utils.basic.reduce_masked_mean(
                        feat, valid, dim=reduce_axis
                    )
            ...
            if not pca:
                ...
                return self.summ_rgb(
                    name, feat_pca, only_return=only_return, frame_id=frame_id
                )
    ...
    def draw_traj_on_image_py(
        ...
        if maxdist is not None:
            ...
            color = (
                np.array(color_map((S1 - 1) / max(1, float(S - 2)))[:3]) * 255
            )  # rgb
