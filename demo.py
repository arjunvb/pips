...
def run_model(model, rgbs, N, sw):
    ...
    grid_y, grid_x = utils.basic.meshgrid2d(
        B, N_, N_, stack=False, norm=False, device="cuda"
    )
    ...
    pad = 50
    rgbs = F.pad(
        rgbs.reshape(B * S, 3, H, W), (pad, pad, pad, pad), "constant", 0
    ).reshape(B, S, 3, H + pad * 2, W + pad * 2)
    trajs_e = trajs_e + pad
    ...
    if sw is not None and sw.save_this:
        ...
        # visualize the input
        o1 = sw.summ_rgbs(
            "inputs/rgbs", utils.improc.preprocess_color(rgbs[0:1]).unbind(1)
        )
        ...
        # alternate vis
        sw.summ_traj2ds_on_rgbs2(
            "outputs/trajs_on_rgbs2",
            trajs_e[0:1],
            vis_e[0:1],
            utils.improc.preprocess_color(rgbs[0:1]),
        )
        ...
        for trajs_e_ in preds_anim:
            ...
            rgb_vis.append(
                sw.summ_traj2ds_on_rgb(
                    "",
                    trajs_e_[0:1],
                    torch.mean(utils.improc.preprocess_color(rgbs[0:1]), dim=1),
                    cmap="spring",
                    linewidth=linewidth,
                    only_return=True,
                )
...
def main():
    ...
    exp_name = (
        "00"  # (exp_name is used for logging notes that correspond to different runs)
    )
    ...
    writer_t = SummaryWriter(
        log_dir + "/" + model_name + "/t", max_queue=10, flush_secs=60
    )
    ...
    while global_step < max_iters:
        ...
        try:
            ...
            print(
                "%s; step %06d/%d; rtime %.2f; itime %.2f"
